...
// import HoursModel from "../models/HourSubmission.ts";
// import EventModel from "../models/Event.ts";
// import AttendanceModel from "../models/Attendance.ts";
...
// const password = encodeURIComponent(<string>process.env.PASS);
...
//   try {
//     let user = await UserModel.findById(id, "name email _id");
//     done(null, user);
//   } catch (error) {
//     done(error, null);
//   }
...
//       try {
//         const user = await UserModel.findOne({ user_id: profile.id });
...
app.get(
  ...
  async function (req, res) {
    req.session.user = await UserModel.findById(req.session.passport.user._id);
...
app.get("/getData", async (req, res) => {
  const result = await HoursModel.find({ person: req.session.user._id });
...
app.post("/add", async (req, res) => {
  ...
  let submission = new HoursModel({
    date: data.date,
    person: req.session.user._id,
    numHours: data.numHours,
    reason: data.reason,
  });
  await HoursModel.save();
...
app.post("/update", async (req, res) => {
  ...
  data.map(async (item) => {
    const result = await HoursModel.updateOne(
      { _id: item._id },
      {
        $set: {
...
app.post("/addAttendance", async (req, res) => {
  ...
  await AttendanceModel.save(attendance);
